...
import { ShieldCheck, Zap, Sparkles, Flame, Star, Trophy, Clock, Filter, ArrowRight } from 'lucide-react';
import { Navbar } from '../components/Navbar';
import { motion, AnimatePresence } from 'motion/react';
import { Link } from 'react-router-dom';

export function Home() {
  const { apps, searchQuery, setSearchQuery } = useStore();
    ...
    <div className="min-h-screen flex flex-col bg-transparent">
      ...
      <div className="relative overflow-hidden border-b border-zinc-900 bg-zinc-950/25">
        ...
        <div className="relative max-w-7xl mx-auto px-4 md:px-8 py-20 pb-24">
          ...
          >
            <div className="flex items-center gap-2.5 text-xs text-zinc-400 bg-zinc-900/60 backdrop-blur-md px-4 py-2.5 rounded-xl border border-zinc-800/80">
      ...
      {/* 📌 Community Highlights Shelf - Recém-Publicados em Destaque */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {recentCommunityApps.map((app) => (
              <motion.div
                ...
                className="relative bg-gradient-to-b from-zinc-900/40 to-zinc-950/80 border border-zinc-850/80 hover:border-mk-green-500/40 rounded-2xl p-5 group shadow-[0_10px_30px_rgba(0,0,0,0.3)] transition-all"
              >
                ...
                <div className="pt-3 border-t border-zinc-900/80 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-zinc-500 font-mono">Cloudflare Link</span>
                  </div>
                  <Link 
                    to={`/app/${app.id}`}
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-8 border-b border-zinc-900 mb-10">
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-3 lg:pb-0 scrollbar-none max-w-full">
            <div className="flex items-center gap-1.5 bg-zinc-950 p-1.5 border border-zinc-850 rounded-2xl">
              <div className="p-1 px-2.5 text-zinc-600 text-xs font-mono font-bold flex items-center gap-1 tracking-wider uppercase">
                <Filter size={12} />
                <span>Filtro:</span>
              </div>
          ...
          <div className="flex items-center gap-2.5 self-end lg:self-center">
            <span className="text-[11px] text-zinc-500 font-bold tracking-wider font-mono uppercase">Ordenar:</span>
